...
NOTE: You may need to first make sure the environment variable `ARGODEMO_AWS_SUBDIRECTORY` is defined (e.g. `ARGOCD_AWS_SUBDIRECTORY="burner/rosa"`)

----
docker run -it -v ~/.kube:/home/jboss/.kube -v ~/.oh-my-zsh:/home/jboss/.oh-my-zsh -v $(pwd):/workspaces/argocd-demo -v ~/.aws:/home/jboss/.aws -e AWS_CONFIG_FILE="/home/jboss/.aws/${ARGODEMO_AWS_SUBDIRECTORY}/config" -e AWS_CREDENTIALS_FILE "/home/jboss/.aws/${ARGODEMO_AWS_SUBDIRECTORY}/credentials" -w /workspaces/argocd-demo --entrypoint /usr/bin/zsh quay.io/mhildenb/argocd-demo-shell
----
...
----
KN_ROUTE=$(oc get ksvc payment -o jsonpath={'.status.url'})
...
+
----
oc get kafkasource kafka-source-orders -o yaml -n argocd-demo-dev
----
+
...
[WARNING]
====
Part of what gets changed is a `ConfigMap` with the name of a (patched) Kafka topic for orders and payments.  Changing the `ConfigMap`, however, will *not* cause the `order-service` pod to be restarted.  This currently needs to be done manually.

If you don't restart the orders pod it will cause orders to be posted to the old `orders` topic which the Kafka Source for the new serverless `payment-service` is not configured to listen to
====
+
